...
- Go 1.26+ (for building from source) or Homebrew
...
This creates a `.unsample/` directory with all config files, including a generated HMAC secret.
...
**Important:** Set `UNSAMPLE_SECRET` to the same value in all services and the CLI. The secret generated by `unsample init` is stored in `.unsample/config.yaml`.
...
The CLI auto-reads the secret from `.unsample/config.yaml` (created by `unsample init`). No need to export environment variables.
